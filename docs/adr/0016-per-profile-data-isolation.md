# ADR 0016: Profiles isolate user data; profile is the first data-partition key
...
ADR 0015 made a profile switch retarget the *agent runtime* (which Hermes
...
2. **Hermes-owned chat sessions** — the real conversation lives in Hermes's
   ...
   (ADR 0015) but has never had a profile-aware *read*: the chat list calls
...
to delete the *active* profile without switching first — ADR 0015.)
...
- **Gateway-per-profile.** Already rejected in ADR 0015 for memory/spawn cost
