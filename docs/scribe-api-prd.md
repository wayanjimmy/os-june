# Scribe API — proxy all upstream AI traffic and fully integrate OS Accounts authorize/charge

> Read [`/CONTEXT.md`](../CONTEXT.md) first for the glossary. Terms in **bold**
> below (Scribe, Scribe API, OS Accounts, upstream provider, dictation, note
...
A user installing Scribe today can record meetings, get transcripts, and
generate notes — but only if **they** supply their own OpenAI and Venice API
keys in a local `.env`. The moment Scribe is distributed as a `.app` to anyone
...
The result: Scribe cannot be shipped publicly as a paid product because there
...
Build **Scribe API**, a confidential backend service that:

1. Holds the OpenAI and Venice API keys (which Scribe no longer ships).
2. Holds the OS Accounts App API key (`osk_…`) that authenticates Scribe to OS
...
After this lands, Scribe ships with **zero upstream provider keys**, every AI
...
1. As a **first-time Scribe user**, I want to install the app without
   configuring any third-party API keys, so that I can get to recording within
   one minute of double-clicking the `.app`.
2. As a **first-time Scribe user**, I want to **sign in with Open Software**
...
3. As a **signed-in Scribe user**, I want to see my current credit balance in
   the Account settings, so that I know what I can spend before recording.
4. As a **signed-in Scribe user**, I want a one-click "Top up" button that
   opens the OS Accounts portal in my browser, so that I can buy more credits
   without leaving Scribe's flow.
5. As a **signed-in Scribe user**, I want my balance in the Account settings
...
6. As a **Scribe user with sufficient credits**, I want to record a meeting,
...
7. As a **Scribe user with sufficient credits**, I want the cost of each
...
8. As a **Scribe user**, I want **note generation** (turning the transcript
   into structured markdown) to also debit the same OS Accounts wallet, so
   that there is one place to see all my AI spend.
9. As a **Scribe user with insufficient credits**, I want to see a clear
...
15. As a **Scribe user**, I want to choose between OpenAI and Venice models
    for transcription in Settings exactly as today, so that the model picker
    UX is preserved.
16. As a **Scribe user**, I want models that Scribe hasn't priced to NOT
    appear in the picker, so that I never select an option that will fail at
    request time.

### Developer — running and contributing to Scribe

17. As a **Scribe developer running the app locally**, I want to point at
    either a local Scribe API or a staging deployment via a single env var,
    so that I can test against production-shape infra without rebuilding.
18. As a **Scribe developer**, I want to clone the repo and build both the
    Tauri client and Scribe API from the same `git clone`, so that
    end-to-end changes are one PR.
19. As a **Scribe developer**, I want no upstream provider keys (`OPENAI_API_KEY`,
    `VENICE_API_KEY`) anywhere in the Tauri-side `.env` or codebase, so that
    a leaked Scribe build cannot drain our provider budgets.

### Developer — Scribe API backend

20. As a **Scribe API maintainer**, I want every paid request to atomically
...
25. As a **Scribe API maintainer**, I want structured `tracing` logs with
...
26. As a **Scribe API maintainer**, I want PRs that touch only the Tauri
    side to NOT trigger Rust CI, so that a UI tweak doesn't run a 5-minute
    cargo build.
27. As a **Scribe API maintainer**, I want pushes to `main` that touch
    `scribe-api/**` to build and publish a `:<short-sha>` image to GHCR plus
    update a `:staging` tag, so that staging always reflects what's on
    `main`.
28. As a **Scribe API maintainer**, I want a manual workflow to re-tag an
    existing image to `:production`, so that promotion never rebuilds and
    rollback is "promote a previous SHA".
29. As an **on-call engineer**, I want Scribe API's `/livez` to return 200
...
30. As an **OS Accounts admin**, I want each metered call from Scribe to
...
31. As an **OS Accounts platform**, I expect Scribe API to never call
...
- **Scribe API lives in this repo**, not a separate repo. Cargo workspace
  rooted at `scribe-api/` (sibling of `src-tauri/` and `src/`). One `git
  clone` for client + backend; CI workflows in the monorepo gate on
  `paths: scribe-api/**`.
- **Six crates, not seven.** The `scribe-persistence` crate from the
  `os-rust-backend` recipe is deliberately omitted — Scribe API is
  stateless v1. If persistence is ever added, it goes in a new
  `scribe-persistence` crate at that time. Crates: `scribe-domain`,
  `scribe-services`, `scribe-providers`, `scribe-config`, `scribe-api`,
  `scribe-app`. Binary is named `scribe`.
- **Service-prefix `scribe`.** All library crate names start with `scribe-`.
...
- **Follow `os-rust-backend` recipe** for everything not explicitly
  ...
  `scribe-config`, error code bands per the standard table.

### Deep modules and their interfaces (no file paths)

- **`Transcriber` (trait, `scribe-domain`)** — `transcribe(audio, model) ->
  Result<Transcript, DomainError>`. Two impls in `scribe-providers`:
  OpenAI and Venice. Interface unchanged across upstreams.
- **`Generator` (trait, `scribe-domain`)** — `generate(prompt_inputs, model)
  -> Result<GeneratedNote, DomainError>`. One impl (Venice) for now.
- **`Cleaner` (trait, `scribe-domain`)** — `cleanup(text, context, style,
  model) -> Result<String, DomainError>`. One impl (Venice).
- **`OsAccountsClient` (trait, `scribe-domain`)** — `authorize(user, action,
  estimate, hold_ttl) -> Result<Authorization, _>` and `charge(token,
  credits, idempotency_key) -> Result<Receipt, _>`. One impl
  (`OsAccountsHttpClient` in `scribe-providers`) — HTTP, App API key
  bearer, envelope-aware, no retries beyond what the platform guarantees.
- **`TokenVerifier` (trait, `scribe-domain`)** — `verify(jwt) ->
  Result<UserId, AuthError>`. One impl (`JwksTokenVerifier` in
  `scribe-providers`) — caches JWKS in-process, refreshes on `kid` miss,
  ES256-only, strict `iss`/`aud`, `exp` with small leeway. Never trust
  the token's `alg` header; never fetch JWKS per request.
- **`PricingTable` (`scribe-services`)** — `price_credits(model_id, units)
  -> Result<Credits, NotPriced>`. Pure function over the config table.
  Single source of truth for credit cost. Loaded from `scribe-config`.
- **`WavDurationProbe` (`scribe-providers`)** — `probe(bytes) ->
  Result<Duration, ProbeError>`. Pure, uses the `hound` crate (already a
  workspace dep in `src-tauri`; should be added to the Scribe API
  workspace too).
- **`NoteTranscribeService`, `NoteGenerateService`, `DictateService`
  (`scribe-services`)** — orchestrators. Each takes a `UserId` (from the
...
- **Tauri-side `scribe_api`** — single module exposing `transcribe`,
  ...
  `Bearer <jwt>` request to `{SCRIBE_API_URL}/v1/…`, on `401` refreshes
...
Deterministic, derived from a stable per-operation identifier in Scribe:
...
The Scribe client passes the stable identifier in the request (`noteId` /
`sessionId+utteranceId`); Scribe API constructs the full key
server-side from the verified `usr_` + the client-supplied operation id.
Clients never compose the full key themselves.

### Configuration

Scribe API (typed `AppConfig` in `scribe-config`, figment-merged from
...
- `[pricing]` — table keyed by `model_id`, each entry `{ unit: "seconds" |
  ...
  into the image; `SCRIBE__PRICING__…` env overrides supported via
  figment for per-env tweaking.

Scribe (Tauri client):

- `SCRIBE_API_URL` — baked production default in a constant; `.env`
...
- `scribe-api.yml` — PR + push to `main`, gated on `paths:
  scribe-api/**`. fmt, clippy, tests, OpenAPI drift (if a doc is derived).
  Never publishes.
- `build-scribe-api.yml` — push to `main` touching `scribe-api/**`, or
  manual dispatch. Builds and pushes `ghcr.io/open-software-network/scribe-api:<short-sha>`
  + updates `:staging`. Stops at GHCR.
- `promote-scribe-api.yml` — manual dispatch. Verifies an existing image,
...
`scribe-api/rust-toolchain.toml` and `changed-paths` configured for the
`scribe-api/` subdirectory.

### Tauri-side refactor

- Add `src-tauri/src/scribe_api.rs` with the functions listed above.
...
- Delete `src-tauri/src/providers/transcription.rs`,
  ...
  Settings into `scribe_api::list_models` (or a small `model_picker.rs`
  cache if a separate cache is wanted).
- Update callers in `commands.rs` and `dictation.rs` to import from
  `scribe_api` instead of `providers::*`.
- Map Scribe API `error_code: 4301` to whatever existing "insufficient
...
1. Register Scribe in the OS Accounts admin console for staging and
...
2. Create Railway projects for Scribe API staging + production.
3. Confirm GHCR `packages: write` on the GitHub Actions token for
   `ghcr.io/open-software-network/scribe-api`.

## Testing Decisions

### What makes a good test for this work

- **Test external behavior, not implementation details.** For Scribe API,
...
- **Tests assert on the envelope, not just the HTTP status.** A
  ...
  should match the same `error_code` Scribe will branch on.

### Modules with tests in scope

- **`PricingTable` (unit, `scribe-services`)** — pure function over the
...
- **`OsAccountsHttpClient` (integration vs `wiremock`, `scribe-providers`)** —
...
- **Streaming responses** (SSE / chunked) for `/v1/generate`. Current
  Scribe UX is non-streaming; add later as its own feature.
- **Postgres / persistence in Scribe API.** No DB v1; `scribe-persistence`
...
- **Splitting `scribe-api/` into its own repo.** Monorepo is the decision
...
- **Building any kind of top-up / checkout UI in Scribe or Scribe API.**
...
- **Glossary alignment.** Throughout the implementation, prefer the
  vocabulary from `CONTEXT.md`: **Scribe** (the app), **Scribe API**
...
- **The seven-crate vs six-crate decision is a real deviation from house
  ...
  `scribe-services` or `scribe-providers`.
- **Hosting target is Railway**, alongside OS Accounts but in a separate
  ...
  by `build-scribe-api.yml`; deployment (how Railway picks up the new
  image) is out of scope of `os-rust-backend-ci` and owned by whoever
  configures the Railway side.
- **The Scribe binary will bake `SCRIBE_API_URL` as a production
  ...
  — the moment Scribe is distributed to non-developers, env-only
...
- **`docs/os-accounts-backend.md` should be updated or removed** once
  ...
  built. Pointing it at `scribe-api/` and this PRD is enough.
